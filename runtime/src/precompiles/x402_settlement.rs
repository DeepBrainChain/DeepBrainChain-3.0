    ...
    T: pallet_evm::Config + pallet_x402_settlement::Config,
{
    fn query_payment_intent(handle: &mut impl PrecompileHandle) -> PrecompileResult {
        ...
        let (exists, settled) = match intent {
            Some(p) => {
                let settled =
                    matches!(p.status, pallet_x402_settlement::PaymentIntentStatus::Settled);
                (true, settled)
            },
            None => (false, false),
        };
        let encoded = ethabi::encode(&[ethabi::Token::Bool(exists), ethabi::Token::Bool(settled)]);
    ...
    fn query_settlement_receipt(handle: &mut impl PrecompileHandle) -> PrecompileResult {
        ...
        let encoded = ethabi::encode(&[ethabi::Token::Bool(exists)]);
        Ok(PrecompileOutput { exit_status: ExitSucceed::Returned, output: encoded })
    }
}
