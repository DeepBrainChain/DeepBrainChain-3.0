...
#[derive(PartialEq, Eq, Clone, Encode, Decode, DecodeWithMemTracking, sp_debug_derive::RuntimeDebug)]
...
impl<AccountId> Default for Summary<AccountId> {
    fn default() -> Self {
        Self {
            valid_vote: vec![],
            invalid_vote: vec![],
            unruly: vec![],
            info: None,
            verify_result: VerifyResult::default(),
        }
    }
}
