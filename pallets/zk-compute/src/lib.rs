...
pub mod pallet {
    ...
    pub type MinerPendingCount<T: Config> =
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn submit_proof(
            ...
            let next_task_id =
                task_id.checked_add(&One::one()).ok_or(Error::<T>::TaskIdOverflow)?;
        ...
        pub fn verify_task(origin: OriginFor<T>, task_id: T::TaskId) -> DispatchResult {
            ...
            Self::deposit_event(Event::ProofVerified { task_id, verifier, status: task.status });
        ...
        pub fn claim_reward(origin: OriginFor<T>, task_id: T::TaskId) -> DispatchResult {
            ...
            Self::deposit_event(Event::RewardClaimed { task_id, miner, reward });
            Ok(())
        }
        ...
        pub fn submit_verification_unsigned(
            ...
            if verified {
                let _ = VerifiedTasks::<T>::try_mutate(|history| history.try_push(task_id));
                Self::increase_score(&task.miner);
            } else {
                Self::decrease_score(&task.miner);
            ...
            Self::deposit_event(Event::ProofVerifiedByOcw { task_id, verified });
    ...
    impl<T: Config> Pallet<T> {
        fn ocw_verify_pending_tasks() -> Result<(), Error<T>> {
            ...
            for task_id in pending.iter() {
                if let Some(task) = Tasks::<T>::get(task_id) {
                    ...
                    let call = Call::submit_verification_unsigned { task_id: *task_id, verified };
        ...
        fn valid_dimensions((m, n, k): (u32, u32, u32)) -> bool {
            ...
            let volume = (m as u128).checked_mul(n as u128).and_then(|v| v.checked_mul(k as u128));
            volume.is_some()
        }
        ...
        fn calculate_reward(task: &ZkTaskOf<T>) -> Result<BalanceOf<T>, Error<T>> {
            ...
            let numerator = base_reward_u128
                ...
                .ok_or(Error::<T>::ArithmeticOverflow)?;
            let reward_u128 =
                numerator.checked_div(100_000_000u128).ok_or(Error::<T>::ArithmeticOverflow)?;
        ...
        fn decrease_score(miner: &T::AccountId) {
            let score =
                Self::current_miner_score(miner).saturating_sub(T::ScorePenaltyOnFailure::get());
            MinerScores::<T>::insert(miner, score);
            Self::deposit_event(Event::MinerScoreUpdated { miner: miner.clone(), score });
        }
    }
