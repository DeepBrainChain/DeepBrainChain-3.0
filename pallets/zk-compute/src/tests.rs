use crate::{
    mock::{
        new_test_ext, Balances, RuntimeOrigin, SubmissionDeposit, System, Test,
        VerificationTimeout, ZkCompute,
    },
    Error, Event, MinerScores, ZkVerificationStatus,
};
...
fn verify_task_should_mark_failed_and_slash_deposit() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            Balances::free_balance(ZkCompute::account_id()),
            pallet_before + SubmissionDeposit::get()
        );
