use crate::{
    mock::{new_test_ext, Balances, RuntimeOrigin, System, Test, X402Settlement},
    pallet::PaymentIntentStatus,
};
use codec::Encode;
use frame_support::{
...
fn verify_settlement_fails_for_wrong_status() {
    new_test_ext().execute_with(|| {
        ...
        // Verify once
        assert_ok!(X402Settlement::verify_settlement(RuntimeOrigin::signed(100), intent_id));
...
fn finalize_settlement_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(X402Settlement::finalize_settlement(RuntimeOrigin::signed(1), intent_id));
        ...
        assert_eq!(Balances::free_balance(1), 999_999_000_000);
        assert_eq!(Balances::free_balance(3), miner_balance_before + 1_000_000);
...
fn finalize_settlement_by_miner_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(X402Settlement::finalize_settlement(RuntimeOrigin::signed(3), intent_id));
...
fn finalize_settlement_by_facilitator_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(X402Settlement::finalize_settlement(RuntimeOrigin::signed(100), intent_id));
...
fn fail_payment_intent_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(X402Settlement::fail_payment_intent(RuntimeOrigin::signed(100), intent_id));
